...
import { ensureTodayCelebrationNotifications } from "@/lib/fevcara/celebrationEvents";
...
type CelebrationNotificationRow = {
  id: string;
  title: string;
  body: string;
  link_path: string | null;
...
export default async function AppHomePage() {
  ...
  try {
    await ensureTodayCelebrationNotifications({
      supabase,
      userId: user.id,
    });
  } catch (error) {
    console.error("Home celebration notification error:", error);
  }


  const { data: celebrationNotificationData } = await supabase
    .from("notifications")
    .select("id, title, body, link_path, related_thread_id, related_character_id, created_at")
    .eq("user_id", user.id)
    .eq("type", "celebration_day")
    .is("read_at", null)
    .order("created_at", { ascending: false })
    .limit(1)
    .maybeSingle();
  ...
  const celebrationNotification =
    (celebrationNotificationData ?? null) as CelebrationNotificationRow | null;

  const celebrationChatHref =
    celebrationNotification?.link_path ||
    (celebrationNotification?.related_thread_id
      ? `/app/chat/${celebrationNotification.related_thread_id}`
      : "/app/chats");
    ...
    <main className="min-h-screen bg-[radial-gradient(circle_at_top_left,rgba(190,242,100,0.22),transparent_34%),radial-gradient(circle_at_top_right,rgba(125,211,252,0.22),transparent_34%),linear-gradient(180deg,#FFFFFF_0%,#F8FAFC_54%,#F1F5F9_100%)] px-5 pb-28 pt-8 text-[#1E293B]">
      <section className="mx-auto w-full max-w-md">
        ...
        {celebrationNotification ? (
          <section className="mt-5 overflow-hidden rounded-[2rem] border border-[#FACC15]/35 bg-[#0F172A]/85 p-5 shadow-2xl shadow-[#FACC15]/10">
            <div className="flex items-start justify-between gap-4">
              <div className="min-w-0">
                <p className="text-sm font-black tracking-[0.16em] text-[#FACC15]">
                  SPECIAL MESSAGE
                </p>
                <h2 className="mt-2 text-xl font-black leading-tight text-white">
                  {celebrationNotification.title}
                </h2>
              </div>

              <span className="relative flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl border border-[#FACC15]/25 bg-[#FACC15]/10 text-xl shadow-lg shadow-[#FACC15]/10">
                ✨
                <span className="absolute -right-1 -top-1 h-4 w-4 rounded-full border border-[#0F172A] bg-[#BEF264]" />
              </span>
            </div>

            {celebrationNotification.body ? (
