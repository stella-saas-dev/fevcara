...
# 今回の会話モード
今回は通常の相談・会話モードです。
キャラクター性を出しつつ、必要なときは実用的に答えてください。
ただし、グループ全員が同じ結論を言い換えることは禁止です。
`.trim();
  }
...
- ユーザーへの評価や助言より、自分の好み・今食べたいもの・小さなリアクション・軽いツッコミを優先してください。
- 2人目以降は、直前キャラに絡みつつ、自分の答えも少しだけ混ぜてください。
- みんなで同じことを真面目に言うのではなく、少しテンポの違う会話にしてください。
- くだけた相づち、軽い驚き、ちょっとした冗談はOKです。
- ただし、キャラクターの口調と関係性は守ってください。

良い雰囲気の例:
「それ絶対ごはん進むやつだね。私は今日はあったかいスープ系に逃げたいかも。」
「陽介、肉の話になると急に元気だね。私は卵焼き乗せたい派！」

避ける雰囲気の例:
「その日の体調や気分の着地点を探る作業になるだろう。」
「これは夕飯の中身を確認するだけではなく、余力を把握する行為です。」
`.trim();
}
...
# 今回の絡み先
あなたはこのターンの最初の発言者です。
ユーザーに短く反応しつつ、他キャラクターが会話に入れる余白を残してください。
`.trim();
  }
...
直前の発言者「${previousCharacterMessage.speakerName}」に反応してください。
...
- 2人目以降は、まず直前のキャラクター名に触れ、直前キャラクターへの反応として書いてください。
- 2人目以降は、ユーザーへの直接アドバイスを繰り返すのではなく、前のキャラへの反応・補足・ツッコミ・別解釈を主目的にしてください。
  ...
  if (thread.chat_type === "group") {
    ...
    try {
      ...
      for (let index = 0; index < speakers.length; index += 1) {
        ...
        const latestUserMessage = getLatestUserMessage(workingMessages);
        const isCasualTurn = isCasualGroupUserMessage(
          latestUserMessage?.content || "",
        );
        ...
        let aiReply = sanitizeAiReplyContent({
          ...
          maxSentences: isCasualTurn ? 2 : 2,
          maxCharacters: isCasualTurn ? 170 : 220,
        });
        ...
        characterMessageRows.push({
          ...
          metadata: {
            ...
            generation_rule: isCasualTurn
              ? "lively_banter_character_interaction"
              : "direct_character_interaction_no_duplicate_user_reply",
          },
        });
